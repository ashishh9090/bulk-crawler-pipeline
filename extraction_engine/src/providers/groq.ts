...
export class GroqLlamaAdapter extends BaseProviderAdapter {
  ...
  constructor(params: {
    ...
    super({
      ...
      baseUrl: params.baseUrl || 'https://api.groq.com/openai/v1',
  ...
  async complete(
    ...
    if (!this.apiKey) {
      throw new ProviderError({
        message: 'Missing GROQ_API_KEY for groq-llama3 adapter',
        provider: this.name,
    ...
    try {
      ...
      if (!messageContent) {
        throw new ProviderError({
          message: 'Groq returned empty choices response',
          provider: this.name,
          model,
          latencyMs,
          requestId: options.requestId,
